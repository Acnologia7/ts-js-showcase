...
describe('doesAlertInputsExist', () => {
  ...
  it('should throw an error if sender is missing', () => {
    expect(() => doesAlertInputsExist(undefined, '25')).toThrow(createError(400, 'Sender and age are required'));
  });

  it('should throw an error if age is missing', () => {
    expect(() => doesAlertInputsExist('testSender', undefined)).toThrow(createError(400, 'Sender and age are required'));
  });

  it('should throw an error if both sender and age are missing', () => {
    expect(() => doesAlertInputsExist(undefined, undefined)).toThrow(createError(400, 'Sender and age are required'));
  });
});
...
describe('validateAlertId', () => {
  ...
  it('should throw an error if alertId is not a valid integer', () => {
    expect(() => validateAlertId('abc')).toThrow(createError(400, 'Invalid alert ID'));
  });

  it('should throw an error if alertId is zero', () => {
    expect(() => validateAlertId('0')).toThrow(createError(400, 'Invalid alert ID'));
  });

  it('should throw an error if alertId is a negative number', () => {
    expect(() => validateAlertId('-10')).toThrow(createError(400, 'Invalid alert ID'));
  });

  it('should throw an error if alertId is a decimal number', () => {
    expect(() => validateAlertId('3.14')).toThrow(createError(400, 'Invalid alert ID'));
  });

  it('should throw an error if alertId is undefined', () => {
    expect(() => validateAlertId(undefined as any)).toThrow(createError(400, 'Invalid alert ID'));
  });
});
...
describe('validateFileIdsToDelete', () => {
  it('should return an array of numbers when given an array of numeric strings', () => {
    expect(validateFileIdsToDelete(['1', '2', '3'])).toEqual([1, 2, 3]);
  });

  it('should throw an error if deleteFileIds is not an array', () => {
    expect(() => validateFileIdsToDelete('not-an-array' as any)).toThrow(
      createError(400, 'deleteFileIds must be an array')
    );
  });

  it('should throw an error if any element in deleteFileIds is not a number', () => {
    expect(() => validateFileIdsToDelete(['1', 'two', '3'])).toThrow(
  ...
  it('should throw an error if an element is a valid string but not a numeric string', () => {
    expect(() => validateFileIdsToDelete(['1', '', '3'])).toThrow(
  ...
  it('should throw an error if any element is a decimal or non-integer', () => {
    expect(() => validateFileIdsToDelete(['1', '2.5', '3'])).toThrow(
      createError(400, 'Invalid file ID: "2.5" is not a positive integer')
    );
  });

  it('should throw an error if an element is undefined', () => {
    expect(() => validateFileIdsToDelete(['1', undefined as any, '3'])).toThrow(
      createError(400, 'Invalid file ID: "undefined" is not a positive integer')
    );
  });
});
...
describe('validateFileCount', () => {
  ...
  it('should throw an error if the total file count exceeds MAX_FILES_ALLOWED', () => {
    expect(() => validateFileCount(1, 3, 2)).toThrow(
      createError(400, `Cannot upload more than ${MAX_FILES_ALLOWED} files in total, already at limit`)
    );
  });
